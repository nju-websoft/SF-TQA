...
def parse_args():
    ...
    parser.add_option('-f',dest="offset",type=int,help="id offset")
...
if __name__ == "__main__":
    ...
    for i,sample in enumerate(dataset):
        format_sample = {"id":i+opt.offset,"question":sample["Question"]}
        format_sample["answers"] = [answer.lower() for answer in sample["Gold answer"]]
        format_sample["entity_linking"] = id2el[sample["Id"]]
        format_sample["labels"] = []
        format_sample["temporal_relations"] = []
        if sample["Question"] in question2annotate:
                    ...
                    if label["label"] == "T1":
                        ...
                        for time_annotate in time_annotates:
                            if label["mention"] in time_annotate["text"] or time_annotate["text"] in label["mention"]:
                                ...
                                label["interval"][0] = label["interval"][0].replace("Z","")
                                label["interval"][1] = label["interval"][1].replace("Z","")
