...
from query.freebase_element import Event, Property, TemporalEntity, extend_graph_with_temporal_predicate
...
class BRFonFreebase():#约束映射成实体
    def __init__(self, tlink2algebra_file="data/tlink2algebra.txt"):
        self.frame_name = "BasicReasoningFrame"
        self.tlink2algebra = {}
        with open(tlink2algebra_file, "rt", encoding="utf-8") as fin:
            for line in fin:
                line = line.strip()
    ...
    def generate_onehop_bone(self,parsed_question, limit=canidate_num ):
        ...
        for entity in entities:
            out_paths = query_one_or_twohop_out_paths(entity)
...
class AFonFreebase(BRFonFreebase):
    def __init__(self, temporal_relation, candidate_relations):
        ...
        if(temporal_relation.related.type == "T1"):
            ...
            self.related = Event(temporal_relation.related,interval=temporal_relation.related.interval)
        else:
    ...
    def template_grounding(self, parsed_question):
        ...
        for entity in entities:
            rawgraph = graph.copy()
            graph_list += self.target.grounding_with_time(entity=entity, rawgraph=rawgraph,  question=parsed_question.question)
        if(self.related.interval):
            ...
            rawgraph = graph.copy()
            related_graphs = self.related.grounding_for_related_event_with_time(parsed_question, rawgraph)
                    ...
                    for value_relation in self.candidate_relations:
                        ...
                        for edge in related_graph.edges:
                            ...
                            if edge.type == Edge.URI:
                                new_graph.add_triple(from_val, edge.value, to_val)
                            else:
                                new_graph.add_time_interval(from_val, edge.value, to_val)
                        # relation filter implemention
                        new_graph.add_time_value_relation(target_var, value_relation, related_var)
                        candidate_graphs.append(new_graph)
        return candidate_graphs

class TAsFonFreebase():
    ...
    def template_grounding(self,parsed_question):
        ...
        graph_list = []
        for entity in entities:
            graph_list += self.target.grounding(entity, rawgraph, parsed_question.question, limit=canidate_num)
        for graph in graph_list:
            target_graphs = extend_graph_with_temporal_predicate(graph,var="ans")
            for graph, time_var in target_graphs:
                graph.add_time_value_relation(time_var, TIME_VALUE_RELATION.EQUAL, self.time)#"ns:measurement_unit.dated_integer.yaer"
                graph.set_answer_vars(["ans"])
                graphs.append(graph)
        return graphs

class SFonFreebase(BRFonFreebase):
    def __init__(self, temporal_relation):
        self.frame_name = "SuccessivelFrame"
        self.type = temporal_relation.rel_type
        self.target = Event(temporal_relation.target)
        self.related = Event(temporal_relation.related)
        if(self.type=="BEFORE"):
            self.rank = -1
        else:
            self.rank = 1
        ...
        for graph in raw_graphs:
            for edge in graph.edges:
                if(edge.type == Edge.TIME_VALUE_RELATION):
                    ...
                    new_graph.set_frame_name(self.frame_name)
                    from_val = graph.find_node(edge.from_id).value
                    new_graph.add_temporal_order(from_val, self.rank)
                    graphs.append(new_graph)
        return graphs

class STFonFreebase(BRFonFreebase):
    def template_grounding(self, parsed_question):
        trigers = ["follows", "preceded by", "followed by", "precedes", "preceded"]
        graphs = []
        ...
        for graph in entity_graphs:
            ...
            for triger in trigers:
                if (triger in graph.to_label_seq().lower()):
                    graphs.append(graph)
        return graphs

class POFonFreebase(BRFonFreebase):
    def __init__(self,temporal_relation):
        self.target = Event(temporal_relation.target)
    ...
    def template_grounding(self,parsed_question):
        ...
        raw_graph.set_frame_name(self.frame_name)
        graph_list = []
        target_graphs = []
        if (len(entities) == 2):
            ##statement
            graphs = self.target.grounding_for_two_entities(entities[0], entities[1], raw_graph)
            for graph in graphs:
                is_part = False
                for edge in graph.edges:
                    if(edge.value in part_of_predicates):
                        is_part = True
                        break
                if(is_part):
            ...
            if(len(graph_list)==0 and len(target_graphs)>0):
                event_entity = [e for e in entities if is_time_event(e)]
...
class SiFonFreebase(BRFonFreebase):
    '''
    ...
    def __init__(self, temporal_relation):
        ...
        self.target = Event(temporal_relation.target)
        self.related = TemporalEntity(temporal_relation.related)
        self.time = None
        if (temporal_relation.related.type == "T1"):
    ...
    def template_grounding(self, parsed_question):
        ...
        if (self.time):
            ...
            for entity in entities:
                if (self.related_text and self.related_text in query_label(entity)):
                    ...
                    break
                time_res = get_time_around_entity(entity)
        ...
        if (len(graph_list) >canidate_num):
            ...
            graph_list.sort(key=lambda g: g.score, reverse=True)
            graph_list = graph_list[:canidate_num]
        return filter_graphs(graph_list)

class TAsFOonFreebase(BRFonFreebase):
    ...
    def template_grounding(self,parsed_question):
        ...
        if(self.constraint_type=="predicate"):
            for graph in entity_graphs:
                graph.set_frame_name(self.frame_name)
                labels = ""
                for pred in list(graph.predicates):
                    labels += query_label(pred)+" "
                if(self.rank==1):
                    if("first" in labels):
                        graphs.append(graph)
            ...
            for graph in entity_graphs:
                ...
                for var in graph.vars:
                    ...
                    tmp_graph = graph.copy()
                    tmp_graph.add_triple(var, "p", var_name)
                    ...
                    for result in results:
                        prop = result["p"]
                        tmp_graph = graph.copy()
                        tmp_graph.add_triple(var, prop, var_name)
                        tmp_graph.add_numerical_cmp(var_name, "=", self.rank)
                        graphs.append(tmp_graph)
        return graphs

class OFonFreebase(BRFonFreebase):
    def __init__(self,temporal_ordinals,constraint_type=None):##constraint_type="num" or "value" or "predicate"
    ...
    def template_grounding(self,parsed_question):
        ...
        for entity in entities:
            rawgraph = graph.copy()
            graph_list += self.target.grounding(entity=entity, rawgraph=rawgraph, question=parsed_question.question)
        for graph in graph_list:
...
class TAFonFreebase(BRFonFreebase):
    ...
    def template_grounding(self,parsed_question):
        ...
        if(len(parsed_question.rec_numerical_cmp_cons_list)>0):
            ...
            for entity in entities:
                ...
                for graph in graph_list:
                    graph.add_numerical_cmp("num", numerical_cmp_cons.cmp_sign, numerical_cmp_cons.number)
                    graph.add_triple("s0", get_uri("ns:measurement_unit.dated_integer.year"), "ans")
                    graph.add_answer_var("ans")
                    graphs.append(graph)
            return graphs
